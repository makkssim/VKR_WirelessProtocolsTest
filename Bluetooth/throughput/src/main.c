			   ...
			   struct bt_conn_le_phy_info *param)
{
	...
	tx_power.phy = param->tx_phy;
	bt_conn_le_get_tx_power_level(default_conn, &tx_power);
	printk("\nCurrent phy: %d \n", tx_power.phy);
	     ...
	     const struct bt_conn_le_data_len_param *data_len)
{
	...
	err = bt_throughput_write(&throughput, datas, 1);
	if (err) {
	...
	if (IS_ENABLED(CONFIG_BT_THROUGHPUT_FILE)) {
		while (*img_ptr) {
			err = bt_throughput_write(&throughput, datas, 495);
			if (err) {
