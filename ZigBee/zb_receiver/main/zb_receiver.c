...
static int16_t firstNum = 32000;
static int16_t previousNum = 0;
static int16_t correct = 0;
static int16_t incorrect = 0;
static int16_t total= 0;
static int16_t sensAddr= 0;
...
static void bind_cb(esp_zb_zdp_status_t zdo_status, void *user_ctx)
{
    ...
    if (zdo_status == ESP_ZB_ZDP_STATUS_SUCCESS) {
        if (bind_req->req_dst_addr == esp_zb_get_short_address()) {
            ESP_LOGI(TAG, "Successfully bind sender from address(0x%x) on endpoint(%d)",
        ...
        if (bind_req->req_dst_addr == temp_sensor.short_addr) {
            ESP_LOGI(TAG, "Sender from address(0x%x) on endpoint(%d) successfully binds us",
                     temp_sensor.short_addr, temp_sensor.endpoint);
        }
        free(bind_req);
    }
...
static void user_find_cb(esp_zb_zdp_status_t zdo_status, uint16_t peer_addr, uint8_t peer_endpoint, void *user_ctx)
{
    if (zdo_status == ESP_ZB_ZDP_STATUS_SUCCESS) {
        ...
        ESP_LOGI(TAG, "Request sender to bind us");
...
void esp_zb_app_signal_handler(esp_zb_app_signal_t *signal_struct)
{
    ...
    case ESP_ZB_ZDO_SIGNAL_DEVICE_ANNCE:
        ...
        sensAddr = dev_annce_params->device_short_addr;
...
static void esp_app_zb_attribute_handler(uint16_t cluster_id, const esp_zb_zcl_attribute_t* attribute)
{
    ...
    if (cluster_id == ESP_ZB_ZCL_CLUSTER_ID_TEMP_MEASUREMENT) {
        if (attribute->id == ESP_ZB_ZCL_ATTR_TEMP_MEASUREMENT_VALUE_ID &&
            ...
            {               
                incorrect += value - previousNum;
                total += value - previousNum;                
            }
            previousNum = value;
            //ESP_LOGI(TAG, "Stats - coorect/incorrect/total packs:%d/%d/%d", correct, incorrect, total);
            ESP_LOGI(TAG, "Packet %d RSSI: %ddbm",value,esp_ieee802154_get_recent_rssi());
            //ESP_LOGI(TAG, "RSSI: %ddbm",esp_ieee802154_get_recent_rssi());
        }
    }
}
...
static void esp_zb_task(void *pvParameters)
{
    ...
    esp_zb_device_register(esp_zb_thermostat_ep);//создание устройства на основе стандартного термостата
