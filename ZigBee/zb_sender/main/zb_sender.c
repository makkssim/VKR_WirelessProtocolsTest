...
static esp_err_t deferred_driver_init(void)
{
    temperature_sensor_config_t temp_sensor_config =
        TEMPERATURE_SENSOR_CONFIG_DEFAULT(0, 1);
    ESP_RETURN_ON_ERROR(temp_sensor_driver_init(&temp_sensor_config, 1, esp_app_temp_sensor_handler), TAG,
...
static void esp_zb_task(void *pvParameters)
{
    ...
    esp_zb_device_register(esp_zb_sensor_ep);//создание устройства на основе стандартного температурного сенсора
...
void app_main(void)
{
    ...
    esp_zb_zcl_report_attr_cmd_t report_attr_cmd;//задание параметров отчета об изменении атрибута
    ...
    while(1){
        ...
        esp_zb_zcl_set_attribute_val(HA_ESP_SENSOR_ENDPOINT,
        ESP_ZB_ZCL_CLUSTER_ID_TEMP_MEASUREMENT, ESP_ZB_ZCL_CLUSTER_SERVER_ROLE,
        ESP_ZB_ZCL_ATTR_TEMP_MEASUREMENT_VALUE_ID, &a, false);//изменение значения атрибута
        esp_zb_lock_release();       

        esp_zb_lock_acquire(portMAX_DELAY);
        esp_zb_zcl_report_attr_cmd_req(&report_attr_cmd);//отправка отчета
        esp_zb_lock_release();
        ESP_EARLY_LOGI(TAG, "Send value: %d",a);
        vTaskDelay(100);
